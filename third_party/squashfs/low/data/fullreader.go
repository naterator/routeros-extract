...
func validBlockSize(size uint32) bool {
	return size >= 4096 && size <= decompress.MaxBlockSize && size&(size-1) == 0
}

func (f FullReader) blockSizeValue(encoded uint32) (uint32, error) {
	if !validBlockSize(f.blockSize) {
		return 0, errors.New("invalid SquashFS block size")
	}
	realSize := encoded &^ (1 << 24)
	if realSize > decompress.MaxBlockSize {
		return 0, errors.New("SquashFS encoded block exceeds the 1 MiB maximum")
	}
	if encoded&(1<<24) != 0 && realSize > f.blockSize {
		return 0, errors.New("SquashFS block exceeds the configured block size")
	}
	return realSize, nil
}
...
func (f *FullReader) AddFragData(blockStart uint64, blockSize uint32, offset uint32) error {
	realSize, err := f.blockSizeValue(blockSize)
	if err != nil {
		return err
	}
	...
	if blockSize == realSize {
		dat, err = decompress.Decode(f.decomp, dat, int(f.blockSize))
		if err != nil {
			return err
		}
	}
	if uint64(offset) > uint64(len(dat)) {
		return errors.New("SquashFS fragment offset exceeds decoded block")
	}
...
func (f FullReader) Block(i uint32) ([]byte, error) {
	...
	realSize, err := f.blockSizeValue(f.sizes[i])
	if err != nil {
		return nil, err
	}
	if realSize == 0 {
...
func (f FullReader) blockFromPool(i uint32) *BlockResults {
	...
	if realSize == f.sizes[i] {
		out.block, out.err = decompress.Decode(f.decomp, out.block, int(f.blockSize))
	}
	return out
}
