...
func ReadFile(r io.Reader, blockSize uint32) (f File, err error) {
	...
	if blockSize == 0 {
		return f, fmt.Errorf("invalid SquashFS block size")
	}
	toRead := uint64(f.Size) / uint64(blockSize)
	if f.FragInd == 0xFFFFFFFF && f.Size%blockSize > 0 {
		toRead++
	}
	if toRead > maxInodeBlockEntries {
		return f, fmt.Errorf("SquashFS file has too many data blocks: %d", toRead)
	}
	dat = make([]byte, int(toRead)*4)
	_, err = r.Read(dat)
	if err != nil {
		return
	}
	f.BlockSizes = make([]uint32, int(toRead))
	for i := range toRead {
