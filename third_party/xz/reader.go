...
type Reader struct {
	...
	stopAtEnd   bool            // local extension: stop before stream padding
	consumed    int64           // bytes actually consumed, excluding read-ahead
}
...
func (z *Reader) Read(p []byte) (n int, err error) {
	...
	for {
		...
		before := z.buf.inPos
		ret := z.decode()
		z.consumed += int64(z.buf.inPos - before)
		switch ret {
		case xzOK:
			// no action needed
		case xzStreamEnd:
			if z.stopAtEnd && z.padding < 0 {
				z.dEOF = true
				break
			}
			if z.padding >= 0 {
